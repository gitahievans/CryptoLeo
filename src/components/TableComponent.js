...
const SaveBtn = ({data}) => {
  ...
  return (
    ...
    <svg className={`w-[1.5rem] ml-1.5 ${allCoins.includes(data.id) ? "fill-cyan" : "fill-gray-100" }
      hover:fill-cyan` }
      ...
      <div className="flex flex-col mt-9 border border-gray-100 rounded">
      {
        ...
        <thead className="capitalize text-base text-gray-100 font-medium border-b border-gray-100">
          <tr>
            ...
            <th className="py-1 ">price(USD)</th>
              ...
              cryptoData.map(data => {
                return(
                  ...
                  <td className="py-4 flex items-center uppercase">
                    ...
                    <img className="w-[1.4rem] h-[1.4rem] mx-1.5" src={data.image} alt={data.name} />
                    <span>
                  ...
                  <td className={
                    data.price_change_percentage_1h_in_currency > 0 ? 'text-green py-4 lg:table-cell hidden':
                    'text-red py-4 lg:table-cell hidden'
                  }>{Number(data.price_change_percentage_1h_in_currency).toFixed(2)}</td>
                  <td className={
                    data.price_change_percentage_24h_in_currency > 0 ? 'text-green py-4 lg:table-cell hidden':
                    'text-red py-4 lg:table-cell hidden'
                  }>{Number(data.price_change_percentage_24h_in_currency).toFixed(2)}</td>
                  <td className={
                    data.price_change_percentage_7d_in_currency > 0 ? 'text-green py-4 lg:table-cell hidden': 
                    'text-red py-4 lg:table-cell hidden'
